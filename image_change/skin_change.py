...
def skin_image(event,userid,color):
    ...
    if color == 1:
        hsv_2[:, :, 0] = np.where((hsv[:, :, 2]>0) & (hsv[:, :, 2]<200) ,hsv[:, :, 0] +50 ,hsv[:, :, 0])#黒(茶色)から緑
       #hsv_2[:, :, 0] = np.where((hsv[:, :, 0]>6) & (hsv[:, :, 0]<30) ,hsv[:, :, 0] + 50,hsv[:, :, 0]) #緑
        ...
        hsv_2[:, :, 0] = np.where((hsv[:, :, 2]>0) & (hsv[:, :, 2]<128) ,hsv[:, :, 0] + 150,hsv[:, :, 0]) #ピンク
        hsv_2[:, :, 1] = np.where((hsv[:, :, 2]>0) & (hsv[:, :, 2]<128) ,hsv[:, :, 1] * 0.5,hsv[:, :, 1])
    elif color == 5:
